...
class Ekspektasi_lab extends CI_Model {
	...
	function get_kuisioner(){
		...
		$check_ada=$this->db->query("SELECT id_pasien from hasil_kuisioner WHERE id_kuisioner=$data_id->id_kuisioner AND id_pasien='0'");
			if ($check_ada->num_rows()>0)
			{
				...
				$this->hasil.="<input type='hidden' class='form-control' name='id_pasien[]' value='0' />";
                $this->hasil.="</div>";
        	    }	
            	$this->hasil.="<hr/>";
                $this->hasil.="<div class='form-group'>";
				...
				$this->hasil.="<input type='hidden' class='form-control' name='kode_pasien' value='0' />";
