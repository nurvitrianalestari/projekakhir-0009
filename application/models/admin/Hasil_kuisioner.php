	...
	function ambil_hasil(){
		...
		if($query->num_rows()>0){
			foreach($query->result_array() as $value){
				...
				$empat = array('jawaban' => '4','id_pasien !=' => '0', 'id_kuisioner' => $value['id_kuisioner']);
				$sangat_setuju= $this->db->where($empat)->count_all_results("detail_jawaban");
				
				$tiga = array('jawaban' => '3','id_pasien !=' => '0', 'id_kuisioner' => $value['id_kuisioner']);
				$setuju= $this->db->where($tiga)->count_all_results("detail_jawaban");

				$dua = array('jawaban' => '2','id_pasien !=' => '0', 'id_kuisioner' => $value['id_kuisioner']);
				$biasa= $this->db->where($dua)->count_all_results("detail_jawaban");
				
				$satu = array('jawaban' => '1','id_pasien !=' => '0', 'id_kuisioner' => $value['id_kuisioner']);
