...
class Hasil_servqual extends CI_Model {
	...
	$query=$this->db->query("SELECT * FROM kriteria WHERE aktif='Y' ORDER BY id_kriteria");
		if ($query->num_rows()>0) {
			...
			$pasien=$this->db->query("SELECT id_pasien FROM hasil_kuisioner WHERE id_pasien !='0' AND id_kuisioner=$id_kuisioner_aktif");
			$jumlah_pasien=$pasien->num_rows();

			$persep_per_kriteria=$this->db->query("SELECT SUM(jawaban) AS j_k_p FROM detail_jawaban WHERE id_kuisioner=$id_kuisioner_aktif AND id_kriteria=$value[id_kriteria] AND id_pasien != '0'");
			foreach ($persep_per_kriteria->result_array() as $key => $persepsi_per_kriteria) {
			}

			$ekspek_per_kriteria=$this->db->query("SELECT SUM(jawaban*$jumlah_pasien) AS j_k_e FROM detail_jawaban WHERE id_kuisioner=$id_kuisioner_aktif AND id_pasien='0' AND id_kriteria=$value[id_kriteria]");
			...
			$hasil_akhir_per_kriteria=@($hasil_per_kriteria/$jumlah_pasien);
	...
	$persepsi= $this->db->query("SELECT SUM(jawaban) AS j_p FROM detail_jawaban WHERE id_kuisioner=$id_kuisioner_aktif AND id_pasien !='0'");
	$jumlah_persepsi= $persepsi->result_array();
	foreach ($jumlah_persepsi as $value) {
	}

	$ekspektasi= $this->db->query("SELECT SUM(jawaban*$jumlah_pasien) AS j_e FROM detail_jawaban WHERE id_pasien='0' AND id_kuisioner=$id_kuisioner_aktif");
	...
	$hasil_servqual=@($jumlah_awal/$jumlah_pasien);
    ...
    {	$kuisioner_aktif=$this->db->query("SELECT * FROM kuisioner WHERE aktif='Y'");
		$aktif_kuisioner=$kuisioner_aktif->row();
		$id_kuisioner=$aktif_kuisioner->id_kuisioner;
        $query = $this->db->query("SELECT COUNT(jawaban) as suara FROM detail_jawaban WHERE id_kuisioner='$id_kuisioner' AND id_pasien !='0' AND  jawaban='4'");
        return $query->result();
    }
    ...
    {	$kuisioner_aktif=$this->db->query("SELECT * FROM kuisioner WHERE aktif='Y'");
		$aktif_kuisioner=$kuisioner_aktif->row();
		$id_kuisioner=$aktif_kuisioner->id_kuisioner;
        return $this->db->query("SELECT COUNT(jawaban) as suara FROM detail_jawaban WHERE id_kuisioner='$id_kuisioner' AND id_pasien !='0' AND jawaban='3'")->result();
    }
    ...
    {	$kuisioner_aktif=$this->db->query("SELECT * FROM kuisioner WHERE aktif='Y'");
		$aktif_kuisioner=$kuisioner_aktif->row();
		$id_kuisioner=$aktif_kuisioner->id_kuisioner;
        return $this->db->query("SELECT COUNT(jawaban) as suara FROM detail_jawaban WHERE id_kuisioner='$id_kuisioner' AND id_pasien !='0' AND jawaban='2'")->result();
    }
    ...
    {	$kuisioner_aktif=$this->db->query("SELECT * FROM kuisioner WHERE aktif='Y'");
		$aktif_kuisioner=$kuisioner_aktif->row();
		$id_kuisioner=$aktif_kuisioner->id_kuisioner;
        $query = $this->db->query("SELECT COUNT(jawaban) as suara FROM detail_jawaban WHERE id_kuisioner='$id_kuisioner' AND id_pasien !='0' AND jawaban='1'");
        return $query->result();
    }
}